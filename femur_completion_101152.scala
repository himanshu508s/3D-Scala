...
object femur_completion_101152 {
  ...
  def main(args: Array[String]) {
    ...
    val femur_partial = MeshIO.readMesh(new java.io.File("data/partials/101152/101152.stl")).get

    val targetGroup = ui.createGroup("target")
    ui.show(targetGroup, femur_partial, "femur_broken")

    val smallModel = StatisticalModelIO.readStatisticalMeshModel(new java.io.File("data/results/37_aligned.h5")).get
   ...
   //  val ssmView = ui.show(modelGroup, model, "femur_refrence")
    ...
    val referenceLandmarks = LandmarkIO.readLandmarksJson[_3D](new java.io.File("data/ref_37_aligned.json")).get
    ...
    val femur_partialLandmarks = LandmarkIO.readLandmarksJson[_3D](new java.io.File("data/partials/101152/101152.json")).get
    ...
    val femur_partialPtIDs = model.referenceMesh.pointSet.pointIds.filter { id =>
      (model.referenceMesh.pointSet.point(id) - model.referenceMesh.pointSet.point(PointId(8690))).norm <= 160
    }
    ...
    StatisticalModelIO.writeStatisticalMeshModel(posterior_femur_partialModel, new java.io.File("data/101152_aligned_final.h5"))

    val meanFemur : TriangleMesh[_3D] = posterior_femur_partialModel.mean
    ui.show(posterior_femur_partialGroup, meanFemur, "meanFemur")
    val filename=new File("Data/results/101152_aligned.stl")
